...
// =====================================================================
// CONFIG / API CLIENT  (NÃO ALTERAR A CONEXÃO)
// =====================================================================
...
async function api<T>(path: string, opts: ApiOpts = {}): Promise<T> {
  ...
  url.pathname = API_BASE.replace(/\/+$/, "") + "/" + path.replace(/^\/+/, "");
...
// =====================================================================
// CSV OVERLAY (opcional) — lê /static/devices.csv do Nginx e pré-preenche
// =====================================================================
...
function _parseCsv(text: string): OverlayRow[] {
  ...
  for (let i=1;i<lines.length;i++){
    ...
    rows.push({
      ...
      model:   _pick(obj, ["model","product_class","modelo","productclass"]),
    });
  }
  return rows;
}
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs opacity-70">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => setPage(Math.max(1, data.page - 1))}>Anterior</Button>
            <Button variant="outline" size="sm" onClick={() => setPage(data.page + 1)}>Próxima</Button>
          </div>
        </div>
      </div>
    </div>
...
async function enrichDetailFallback(id: string, base: DeviceDetail | null): Promise<DeviceDetail> {
  ...
  const out: DeviceDetail = {
    ...
    vendor: base?.vendor ?? v["InternetGatewayDevice.DeviceInfo.Manufacturer"] ?? undefined,
    product_class: base?.product_class ?? v["InternetGatewayDevice.DeviceInfo.ProductClass"] ?? undefined,
    software_version: base?.software_version ?? v["InternetGatewayDevice.DeviceInfo.SoftwareVersion"] ?? undefined,
    ...
    mgmt: {
      conn_req_url: base?.mgmt?.conn_req_url ?? (v["InternetGatewayDevice.ManagementServer.ConnectionRequestURL"] ?? null),
      stun_enable: base?.mgmt?.stun_enable ?? Boolean(v["InternetGatewayDevice.ManagementServer.STUNEnable"]),
      periodic_inform_interval: base?.mgmt?.periodic_inform_interval ?? (typeof v["InternetGatewayDevice.ManagementServer.PeriodicInformInterval"] === "number"
        ? v["InternetGatewayDevice.ManagementServer.PeriodicInformInterval"]
        : (v["InternetGatewayDevice.ManagementServer.PeriodicInformInterval"] ? Number(v["InternetGatewayDevice.ManagementServer.PeriodicInformInterval"]) : null)),
    },
  };
  ...
  useEffect(() => {
    ...
    getDeviceDetail(d.id)
      .then(async (base) => {
        ...
        const [r24, r5] = await Promise.all([deviceGetSSID(d.id, 1), deviceGetSSID(d.id, 2)]);
        enriched.wifi = {
          ssid_24: enriched.wifi?.ssid_24 ?? (r24.ssid || null),
          ssid_5:  enriched.wifi?.ssid_5  ?? (r5.ssid  || null),
        };
        setDetail(enriched);
      ...
      .catch(async () => {
        ...
        const [r24, r5] = await Promise.all([deviceGetSSID(d.id, 1), deviceGetSSID(d.id, 2)]);
        enriched.wifi = { ssid_24: r24.ssid || null, ssid_5: r5.ssid || null };
        setDetail(enriched);
        setSsid24(enriched.wifi?.ssid_24 || "");
        setSsid5(enriched.wifi?.ssid_5 || "");
      })
